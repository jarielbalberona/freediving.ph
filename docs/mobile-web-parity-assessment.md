...
3. Web profile still has broader settings and management depth than mobile, but mobile now has posts, derived dive-site highlights, diving presences/affinities, read-only badges, compact Passport/Journey/Dive Map/Dive Memories summaries, and backend-backed basic profile editing.
...
| Public/member profile view | `/[username]` | `apps/web/src/app/[username]/page.tsx`, `apps/web/src/features/profile/pages/ProfilePage.tsx` | Profile header, tabs, badges, dive map, journey, passport, memories, media/posts | Public profile screen | `src/features/profiles/screens/public-profile-screen.tsx` | Partial | Mobile has header, counts, posts, derived dive-site highlights, diving presences/affinities, read-only badges, and compact proof-based dive identity summaries. Missing bucket list, full profile-experience details, and richer privacy/visibility states. | P0 | Use the new profile identity surface as the base for media deep links, saved/relationship actions, and later full profile-experience detail screens. |
...
| Dive Map/Journey/Passport/Memories | Profile tabs/components | `src/features/profile/components/ProfileDiveMap.tsx`, `ProfileJourney.tsx`, `ProfilePassport.tsx`, `ProfileDiveMemories.tsx`, API in `src/features/profiles/api/profiles.ts` | Rich profile-experience sections and write APIs for journey/memories/passport settings | Compact dive identity summary plus Diving tab | `src/features/profiles/components/profile-dive-identity-summary.tsx`, `src/features/profiles/components/profile-diving-section.tsx`, mobile profile activity query | Partial | Mobile now reads shared Dive Map/Passport/Journey/Dive Memories contracts for a compact proof-based summary. It still lacks native full map/details and write workflows. | P0 | Build first-class detail routes only after media post deep links are complete; keep proof-based counts server-sourced. |
...
- Full Dive Map, Journey, Passport, and Dive Memories detail/write surfaces. Compact read-only summary is now implemented on mobile profiles.
...
| Passport/Journey/Dive Map/Memories | Yes | Yes | Yes | Compact read surface implemented; maps may be simplified before native map | Next work should be detail/deep-link driven, not a premature full native map. |
...
2. Profile parity: read-only badges, compact passport/profile-experience summary, avatar URL, location/home area, certification, and interests are implemented with pending runtime smoke. Remaining gaps are cover/media management, visibility/social settings, and full profile-experience detail/write parity.
...
- Profile parity core is implemented; continue with media post detail/deep links before expanding profile-experience detail surfaces.
