...
- active: Full role-matrix QA remains incomplete. The local pass restored the signed-in dev user to `super_admin` and proved moderation render, but member/moderator/organizer/school-owner/instructor/applicant matrices require role-specific QA identities or a dev-auth mobile runtime harness.
...
- active: Full public-release role-matrix QA remains incomplete for member B, instructor applicant, approved instructor, school owner, event organizer, group owner, super admin, and target user simulator sessions.
- active: Mutation-heavy QA remains incomplete for media comment create/delete, block/unblock, buddy request lifecycle, event organizer payment/check-in review, school owner booking/payment/session review, instructor proof upload/submission, and super-admin-only access.
