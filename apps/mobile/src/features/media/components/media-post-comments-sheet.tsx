import { BottomSheet, Host } from "@expo/ui";
import Ionicons from "@expo/vector-icons/Ionicons";
import type { MediaPostComment } from "@freediving.ph/types";
import { Image } from "expo-image";
import { useState } from "react";
import {
  Pressable,
  ScrollView,
  Text,
  TextInput,
  View,
  useWindowDimensions,
} from "react-native";

import { useCreateMediaPostCommentMutation } from "@/features/media/hooks/use-media-mutations";
import { useMediaPostCommentsQuery } from "@/features/media/hooks/use-media-post-comments-query";
import { LinkedText } from "@/features/shared/links/components/LinkedText";

const relativeTime = (value: string) => {
  const timestamp = new Date(value).getTime();
  if (!Number.isFinite(timestamp)) return "";
  const diffSeconds = Math.max(0, Math.floor((Date.now() - timestamp) / 1000));
  if (diffSeconds < 60) return "now";
  const diffMinutes = Math.floor(diffSeconds / 60);
  if (diffMinutes < 60) return `${diffMinutes}m`;
  const diffHours = Math.floor(diffMinutes / 60);
  if (diffHours < 24) return `${diffHours}h`;
  const diffDays = Math.floor(diffHours / 24);
  if (diffDays < 7) return `${diffDays}d`;
  return new Intl.DateTimeFormat("en", {
    day: "numeric",
    month: "short",
  }).format(new Date(value));
};

const initialsFor = (name: string) =>
  name
    .split(/\s+/)
    .map((part) => part[0])
    .join("")
    .slice(0, 2)
    .toUpperCase();

function MediaPostCommentRow({ comment }: { comment: MediaPostComment }) {
  const name = comment.author.displayName || comment.author.username || "Diver";
  return (
    <View className="flex-row gap-2.5">
      {comment.author.avatarUrl ? (
        <Image
          accessibilityLabel=""
          cachePolicy="memory-disk"
          className="size-8 rounded-full bg-secondary"
          contentFit="cover"
          source={{ uri: comment.author.avatarUrl }}
          transition={120}
        />
      ) : (
        <View className="size-8 items-center justify-center rounded-full bg-primary/10">
          <Text className="text-[11px] font-semibold text-primary">
            {initialsFor(name)}
          </Text>
        </View>
      )}
      <View className="min-w-0 flex-1 border-b border-border/40 pb-3">
        <View className="flex-row items-center gap-2">
          <Text
            className="text-sm font-semibold text-foreground"
            numberOfLines={1}
          >
            {name}
          </Text>
          <Text className="text-xs text-muted-foreground">
            {relativeTime(comment.createdAt)}
          </Text>
        </View>
        <LinkedText className="mt-1 text-sm leading-5 text-foreground" text={comment.body} />
        {comment.likeCount > 0 ? (
          <Text className="mt-1 text-xs font-medium text-muted-foreground">
            {comment.likeCount.toLocaleString()} likes
          </Text>
        ) : null}
      </View>
    </View>
  );
}

export function MediaPostCommentsSheet({
  onClose,
  postId,
  visible,
}: {
  onClose: () => void;
  postId: string | undefined;
  visible: boolean;
}) {
  const { width } = useWindowDimensions();
  const [draft, setDraft] = useState("");

  const commentsQuery = useMediaPostCommentsQuery(postId, { enabled: visible });
  const createComment = useCreateMediaPostCommentMutation(postId);

  const comments = commentsQuery.data?.items ?? [];
  const trimmedDraft = draft.trim();
  const canSend =
    Boolean(postId) && trimmedDraft.length > 0 && !createComment.isPending;

  const sheetWidth = Math.min(width - 28, 430);

  const submitComment = () => {
    if (!canSend) return;

    createComment.mutate(trimmedDraft, {
      onSuccess: () => setDraft(""),
    });
  };

  return (
    <Host matchContents>
      <BottomSheet
        isPresented={visible}
        onDismiss={onClose}
        snapPoints={[{ fraction: 0.6 }, "full"]}
      >
        <View
          className="max-w-full pt-3"
          style={{ flex: 1, width: sheetWidth }}
        >
          <View className="w-full border-b border-slate-900/10 px-4 pb-3">
            <Text className="w-full text-center text-base font-bold text-slate-900">
              Comments
            </Text>
          </View>

          <ScrollView
            className="w-full"
            contentContainerClassName="pb-5 pt-4"
            style={{ flex: 1 }}
          >
            <View className="w-full gap-3">
              {commentsQuery.isLoading ? (
                <Text className="w-full text-center text-sm text-muted-foreground">
                  Loading comments...
                </Text>
              ) : null}

              {commentsQuery.error ? (
                <View className="w-full items-center gap-3 py-8">
                  <Text className="w-full text-center text-sm text-muted-foreground">
                    Comments could not load.
                  </Text>

                  <Pressable
                    accessibilityRole="button"
                    className="min-h-10 items-center justify-center rounded-full bg-secondary/90 px-4"
                    onPress={() => void commentsQuery.refetch()}
                  >
                    <Text className="text-sm font-bold text-slate-900">
                      Try again
                    </Text>
                  </Pressable>
                </View>
              ) : null}

              {!commentsQuery.isLoading &&
                !commentsQuery.error &&
                comments.length === 0 ? (
                <Text className="w-full text-center text-sm text-muted-foreground">
                  Be the first to comment.
                </Text>
              ) : null}

              {!commentsQuery.isLoading && !commentsQuery.error
                ? comments.map((comment) => (
                  <View key={comment.id} className="w-full">
                    <MediaPostCommentRow comment={comment} />
                  </View>
                ))
                : null}
            </View>
          </ScrollView>

          <View className="w-full pt-3">
            {createComment.error ? (
              <Text className="mb-2 text-xs text-red-600">
                {createComment.error.message || "Comment could not be posted."}
              </Text>
            ) : null}

            <View className="w-full flex-row items-end rounded-[28px] bg-secondary/85 py-1 pl-4 pr-2">
              <TextInput
                className="max-h-32 min-h-10 flex-1 py-2.5 pr-3 text-sm leading-5 text-slate-900"
                editable={!createComment.isPending && Boolean(postId)}
                multiline
                onChangeText={setDraft}
                placeholder="Write a comment..."
                placeholderTextColor="#64748b"
                returnKeyType="default"
                style={{ textAlignVertical: "top" }}
                value={draft}
              />

              <Pressable
                accessibilityLabel="Post comment"
                accessibilityRole="button"
                className={`size-10 items-center justify-center rounded-full ${canSend ? "bg-primary" : "bg-transparent"
                  }`}
                disabled={!canSend}
                onPress={submitComment}
              >
                <Ionicons
                  color={canSend ? "#ffffff" : "#64748b"}
                  name="send"
                  size={18}
                  style={{ transform: [{ rotate: "-90deg" }] }}
                />
              </Pressable>
            </View>
          </View>
        </View>
      </BottomSheet>
    </Host>
  );
}
