...
const flattenThreads = (
  pages: { items: MessagingThreadSummary[] }[] | undefined,
): MessagingThreadSummary[] => pages?.flatMap((page) => page.items) ?? [];

const unreadCount = (items: MessagingThreadSummary[]) =>
  items.reduce((sum, item) => sum + item.unreadCount, 0);

const formatCount = (count: number) => (count > 99 ? "99+" : String(count));

const flattenMessages = (
  pages: { items: MessagingThreadMessage[] }[] | undefined,
): MessagingThreadMessage[] => {
  if (!pages) return [];
  return [...pages]
    .reverse()
...
export function MessagingView({ threadId }: { threadId: string | null }) {
  ...
  const currentCategory =
    (searchParams.get("tab")?.toLowerCase() as MessagingThreadCategory) ||
    "primary";
  const category = categories.includes(currentCategory)
    ? currentCategory
    : "primary";
  ...
  const threadMessagesQuery = useThreadMessages(
    activeThreadId,
    canLoadProtectedData,
  );
  const messages = useMemo(
    () => flattenMessages(threadMessagesQuery.data?.pages),
  ...
  const partner =
    threadDetailQuery.data?.participants.find(
      (participant) => participant.id !== session.me?.userId,
    ) ?? threadDetailQuery.data?.participants[0];
  const peerReadMessageId = activeThreadId
    ? realtime.peerReadMessageByThread[activeThreadId]
    : undefined;
        ...
        <div className="grid grid-cols-2 gap-1 rounded-md bg-background p-1">
          {categories.map((tab) => (
            ...
            >
              ...
              <Badge
                variant={tab === category ? "default" : "outline"}
            ...
            return (
              ...
              >
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <p
                    className={`truncate text-xs ${item.hasUnread ? "font-semibold text-foreground" : "text-muted-foreground"}`}
                  >
    ...
    <div className="flex h-full min-h-0 flex-col bg-background">
      <div className="sticky top-0 z-10 flex items-center gap-3 border-b border-border bg-card px-4 py-3">
        ...
        <UserAvatarDetail
          ...
          displayName={
            partner?.displayName || partner?.username || "Conversation"
          }
      ...
      {isActionableRequest ? (
        <div className="border-b border-border bg-amber-50 px-4 py-3">
          <p className="text-sm font-medium text-amber-900">
            This conversation is still in your requests inbox.
          </p>
          <p className="mt-1 text-xs text-amber-800">
            Accept this request to start chatting. Decline hides it from your
            inbox.
          </p>
      ...
      {showOfflineBanner ? (
        <div className="border-b border-destructive/40 bg-destructive/10 px-4 py-2 text-xs text-destructive">
          You are offline. New messages will appear when your connection is
          back.
        </div>
      ) : null}
