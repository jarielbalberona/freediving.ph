...
export default function ExploreSubmissionsPage() {
  ...
  return (
    <AuthGuard
      ...
      description="Submission status is only visible to the owner and moderators."
    >
        ...
        <div className="mx-auto max-w-4xl space-y-6">
          <section className="rounded-4xl border border-border bg-card/90 p-6 shadow-sm">
            <Badge className="rounded-full bg-primary text-primary-foreground">
              Dive site submissions
            </Badge>
            <h1 className="mt-3 font-serif text-4xl tracking-tight text-zinc-950">
              My site submissions
            </h1>
            <p className="mt-2 text-sm text-zinc-600">
              Pending means waiting for review. Hidden means rejected or removed
              from public Explore.
            </p>
          </section>
            ...
            {items.map((item) => (
              <Card key={item.id} className="bg-card/90">
                <CardHeader className="flex flex-row items-start justify-between gap-4">
                  ...
                  <Badge variant={badgeVariant(item.moderationState)}>
                    {item.moderationState}
                  </Badge>
                </CardHeader>
                <CardContent className="space-y-3">
                  <p className="text-sm text-zinc-700">
                    {item.typicalConditions ||
                      "No conditions summary submitted."}
                  </p>
                  ...
                  <Link
                    href={`/explore/submissions/${item.id}`}
            ...
            {!submissionsQuery.isLoading && items.length === 0 ? (
              <Card className="border-dashed">
                <CardContent className="p-6 text-sm text-zinc-600">
                  No submissions yet. Start with a new dive site proposal.
                </CardContent>
              </Card>
            ) : null}
          </div>
