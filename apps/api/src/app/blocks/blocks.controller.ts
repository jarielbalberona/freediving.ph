...
export default class BlocksController extends ApiController {
  ...
  async list() {
    try {
      return this.apiResponse.sendResponse(await this.service.list(this.request.user.id));
    } catch (error: unknown) {
  ...
  async delete() {
    try {
      ...
      if (!check.success) return this.validationError(check.error);
      return this.apiResponse.sendResponse(await this.service.delete(this.request.user.id, blockedUserId, check.data));
    } catch (error: unknown) {
      return this.apiResponse.sendResponse(error as ServiceApiResponse<unknown>);
    }
  }
