...
CREATE INDEX IF NOT EXISTS idx_schools_location_search ON schools (location_source, region_code, province_code, city_code, barangay_code);
...
CREATE INDEX IF NOT EXISTS idx_media_post_saves_user_created_at ON media_post_saves (user_id, created_at DESC, media_post_id DESC);
CREATE INDEX IF NOT EXISTS idx_media_post_comments_post_created_at ON media_post_comments (media_post_id, created_at DESC, id DESC) WHERE deleted_at IS NULL;
...
CREATE UNIQUE INDEX IF NOT EXISTS idx_journey_entries_generated_source ON journey_entries (user_id, source_type, source_id, type) WHERE source_type IS NOT NULL AND source_id IS NOT NULL;
CREATE INDEX IF NOT EXISTS idx_journey_entries_user_cursor ON journey_entries (user_id, occurred_at DESC, id DESC) WHERE state = 'active';
CREATE INDEX IF NOT EXISTS idx_journey_entries_public_cursor ON journey_entries (user_id, occurred_at DESC, id DESC) WHERE state = 'active' AND visibility = 'public';
