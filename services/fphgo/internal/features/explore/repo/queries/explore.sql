...
-- name: HideSiteByID :execrows
UPDATE dive_sites
SET moderation_state = 'hidden',
    moderation_reason = sqlc.narg(moderation_reason),
    reviewed_by_app_user_id = sqlc.narg(reviewed_by_app_user_id),
    reviewed_at = NOW(),
    updated_at = NOW(),
...
WITH inserted AS (
  ...
  SELECT
    ...
    sqlc.arg(proposed_area),
    sqlc.narg(proposed_latitude),
    sqlc.narg(proposed_longitude),
