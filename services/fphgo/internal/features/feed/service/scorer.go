...
func modeMultiplier(mode Mode, itemType ItemType, localBoost float64) (float64, []string) {
	...
	switch mode {
	case ModeFollowing:
		if itemType == ItemTypePost || itemType == ItemTypeCommunityHot {
			multiplier += 0.20
	...
	case ModeNearby:
		...
		if itemType == ItemTypeDiveSpot || itemType == ItemTypeBuddySignal {
			multiplier += 0.15
		}
	case ModeTraining:
		if itemType == ItemTypePost || itemType == ItemTypeBuddySignal || itemType == ItemTypeEvent {
			multiplier += 0.20
			reasons = append(reasons, "training_affinity")
		}
	case ModeSpotReports:
		if itemType == ItemTypeDiveSpot || itemType == ItemTypePost {
			multiplier += 0.30
