...
func TestMigrationsUpDownSanity(t *testing.T) {
	...
	if err := runGoose(t, testDSN, "down"); err != nil {
...
func runGoose(t *testing.T, dsn, direction string) error {
	t.Helper()
	cmd := exec.Command("go", "run", "github.com/pressly/goose/v3/cmd/goose@"+gooseVersion, "-dir", "db/migrations", "postgres", dsn, direction)
	cmd.Dir = ".."
