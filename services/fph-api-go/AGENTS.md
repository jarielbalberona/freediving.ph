...
## Purpose
This service is `fph-api-go`: a Go 1.26 backend using Chi, PostgreSQL (`pgxpool`), per-feature `sqlc`, goose migrations, and single-instance in-memory WebSocket hub. No Redis in v1.
...
- Core commands:
  ...
  - `make migrate-up`
  - `make migrate-down`
- `sqlc` config: `services/fph-api-go/sqlc.yaml`
- DB schema SQL: `services/fph-api-go/db/schema/`
- Goose migrations: `services/fph-api-go/db/migrations/`
